...
const ScatterTooltip = ({ payload }) => {
  ...
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--border2)', padding: '10px 14px', borderRadius: 8, fontSize: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
              ...
              <Scatter data={sorted} onClick={d => slicerActions.toggleStore(d.Store)}>
                {sorted.map((d, i) => (
                  <Cell key={i}
                    ...
                    fillOpacity={isActive(d) ? 0.85 : 0.12}
        ...
        <div>
          <SectionCard title={`Best Stores${slicer.stores.length ? ' (filtered)' : ''}`}>
            <table className="tbl">
              ...
              <tbody>
                {top5.map(d => (
                  <StoreRow key={d.Store} d={d} colorKey="var(--green)"
                    isSelected={slicer.stores.includes(d.Store)}
                    onClick={() => slicerActions.toggleStore(d.Store)} />
                ))}
              </tbody>
            </table>
          </SectionCard>
          <SectionCard title={`Worst Stores${slicer.stores.length ? ' (filtered)' : ''}`}>
            <table className="tbl">
              ...
              <tbody>
                {bot5.map(d => (
                  <StoreRow key={d.Store} d={d} colorKey="var(--red)"
                    isSelected={slicer.stores.includes(d.Store)}
                    onClick={() => slicerActions.toggleStore(d.Store)} />
                ))}
              </tbody>
