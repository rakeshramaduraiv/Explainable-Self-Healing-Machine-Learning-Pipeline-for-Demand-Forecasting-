...
class SequentialPredictor:
    ...
    def process_actuals_upload(self, df: pd.DataFrame) -> dict:
        ...
        if "Date" not in df.columns or "Demand" not in df.columns or "Product" not in df.columns:
            # Try aliases
            rename = {}
            for c in df.columns:
                cl = c.lower().replace(" ", "_")
                if cl in ("sales", "weekly_sales", "units", "quantity") and "Demand" not in df.columns:
                    rename[c] = "Demand"
                if cl in ("date", "week", "order_date") and "Date" not in df.columns:
                    rename[c] = "Date"
                if cl in ("item", "product_id", "item_id", "sku", "category") and "Product" not in df.columns:
                    rename[c] = "Product"
            df = df.rename(columns=rename)

        missing = [c for c in ("Date", "Product", "Demand") if c not in df.columns]
        if missing:
            raise ValueError(f"CSV must have Date, Product, and Demand columns. Missing: {missing}")
        ...
        # Auto-fill missing columns from base data
        base_path = DATA / "uploaded_data.csv"
        if base_path.exists():
            base_df = self._safe_read_csv(base_path)
            last_row = base_df.iloc[-1]
            for col in base_df.columns:
                if col not in df.columns and col not in ("Date", "Demand"):
                    val = last_row.get(col)
                    if val is not None:
                        df[col] = val
                        log.info(f"Auto-filled '{col}' = {val}")
