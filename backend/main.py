...
All existing pipelines are called unchanged from here.
"""
...
def status():
    ...
    if ok["model"]:
        ...
        model_info = {"type": "LightGBM", "trees": int(m.n_estimators_), "features": int(m.n_features_in_)}
...
def metrics():
    ...
    ax.tick_params(colors="#cdd6f4"); ax.xaxis.label.set_color("#cdd6f4")
    ax.yaxis.label.set_color("#cdd6f4"); ax.title.set_color("#cba6f7")
    ax.set_title("Actual vs Predicted"); ax.legend(facecolor="#313244", labelcolor="#cdd6f4")
...
def feature_importance():
    ...
    ax.set_facecolor("#1e1e2e"); fig.patch.set_facecolor("#1e1e2e")
    ax.tick_params(colors="#cdd6f4"); ax.set_title("Top 15 Feature Importances", color="#cba6f7")
    for spine in ax.spines.values(): spine.set_edgecolor("#45475a")
    plt.tight_layout()
...
async def upload(file: UploadFile = File(...)):
    ...
    cols14 = ["item_id","dept_id","cat_id","store_id","state_id","wm_yr_wk",
              "snap_CA","snap_TX","snap_WI","sell_price","dayofweek","weekofyear","month","year","date","sales"]
    cols3  = ["id","date","sales"]
    ...
    for ax in axes: ax.set_facecolor("#1e1e2e"); ax.tick_params(colors="#cdd6f4")
    ...
    if "cat_id" in df.columns:
        ...
        fig3.patch.set_facecolor("#1e1e2e"); ax3.set_facecolor("#1e1e2e")
        ...
        for sp in ax3.spines.values(): sp.set_edgecolor("#45475a")
        plt.tight_layout()
    ...
    if "sell_price" in df.columns:
        ...
        for ax in axes4: ax.set_facecolor("#1e1e2e"); ax.tick_params(colors="#cdd6f4")
    ...
    if "snap_CA" in df.columns:
        ...
        fig5.patch.set_facecolor("#1e1e2e")
        for ax, col, title in zip(axes5, ["snap_CA","snap_TX","snap_WI"], ["CA","TX","WI"]):
            ax.set_facecolor("#1e1e2e"); ax.tick_params(colors="#cdd6f4")
...
def drift():
    ...
    return {"ks_stat": ks, "level": level, "message": msgs.get(level,""), "dist_chart": dist_chart}
...
def predict():
    ...
    # Parse id into human-readable parts
    # id format: HOBBIES_1_001_CA_1_validation
    out["category"] = out["id"].str.split("_").str[0]
    out["dept"]     = out["id"].str.split("_").str[0] + "_" + out["id"].str.split("_").str[1]
    out["item"]     = out["id"].str.split("_").str[:3].str.join("_")
    out["state"]    = out["id"].str.split("_").str[3]
    ...
    ax.set_title("Next Month — Total Daily Forecast (All Products, All Stores)", color="#cba6f7")
    ...
    for i, cat in enumerate(cats):
        ...
        ax2.plot(d["date"], d["predicted_sales"], label=cat, color=cat_colors[i%len(cat_colors)], lw=2)
    ax2.set_title("Forecast by Product Category", color="#cba6f7")
    ax2.set_xlabel("Date", color="#cdd6f4"); ax2.set_ylabel("Units", color="#cdd6f4")
    ax2.tick_params(colors="#cdd6f4")
    ...
    for i, st in enumerate(stores):
        ...
        ax3.plot(d["date"], d["predicted_sales"], label=f"Store {st}", color=cat_colors[i%len(cat_colors)], lw=2)
    ax3.set_title("Forecast by Store", color="#cba6f7")
    ax3.set_xlabel("Date", color="#cdd6f4"); ax3.set_ylabel("Units", color="#cdd6f4")
    ax3.tick_params(colors="#cdd6f4")
